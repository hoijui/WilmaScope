...
without the ability to rotate or zoom the scenes the labels are very
...
visualisation even using current hardware or will
at least aid in the rapid adoption of the paradigm once today's
science fiction becomes tomorrow's reality.
It was with hope of supporting such research that Wilma was conceived
and offered as open
...
from the 3D scene, shrinks the cluster to a
...
We have found a convenient way to determine the radius of a collapsed cluster
...
This method produces collapsed clusters whose size gives the user a
feeling for the number of children it contains but which is still
...
clusters using the toolbar buttons.  The default appearance of the
graph element that will appear may be selected from a menu that
appears when the small arrow on the buttons is clicked, see Figure
\ref{fig-defaultnode}.
Some of these actions are modal.  For example, clicking the ``add
edge'' button causes a message prompting the user to ``select two
nodes to join with an edge''.  Then clicking on a node causes that node
to change colour showing it has been selected.  The user may then
select another node to create the edge or click on the selected node a
...
then select nodes individually and pressing the ``OK'' button creates
...
As previously mentioned Wilma provides a number of different 3D
``widgets'' or ``glyphs'' representing nodes, edges and clusters.
...
interfaces.  Abstract convenience classes implementing these
...
The default
edge glyph is a simple cylinder.  Such simple shapes are useful
...
frame rates when the seen is rotated or zoomed.  The underlying
...
class name is texture mapped onto the surface of the box.  In a
...
boxes are bevelled.  An application which uses cylindrical nodes is
described in Section \ref{sec:fmflow}.  As can be seen in the example,
the top and bottom radius of
...
A very efficient way to represent larger graphs is not to show the
nodes at all and use line segments to show the edges.  3D Graphics
hardware can store a large set of vertices joined by line segments as
a single 3D shape without surface normals.  Not having to render the
surfaces means graphs with tens of thousands of nodes can be viewed in
their entirety.  This style of rendering is shown in Figure
\ref{fig-multiscale}.

In Wilma multiple edges joining a pair of nodes are shown slightly
offset, an example can be seen in Figure \ref{fig-controls}.
...
A more ambitious representation of a cluster might be a complex hull
...
\item it works just as well in three dimensions as in two.
\end{itemize}
...
to give the user a feeling for the size of collapsed clusters, ie even
though their contents is ellided, the space around them gives an
