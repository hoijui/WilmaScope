 ...
 *  rotation direction and speed via a mouse.
 *  <blockquote><pre>
 ...
 *   </pre></blockquote>
 * The above code will add the rotation behavior to the branchgroup
 * group. 
 */
...
/**
 * When initialize, Rotation Behavior will create an {@link AutoRotation} object to rotate
 * the scene graph continuously.
 * @param bg The branch group the AutoRotation object will be attached
 * @param transformGroup The transform group RotationBehavior controls
 * @param bounds The scheduling bounds of the AutoRotation Object 
 */
	public RotationBehavior(BranchGroup bg,TransformGroup transformGroup,Bounds bounds)
	{
		super(transformGroup);
		this.bg=bg;
		//create a AutoRotation Object so that after the left mouse button is released, the scene graph
      ...
      while(criteria.hasMoreElements()){ 
          ...
          event = ((WakeupOnAWTEvent)wakeup).getAWTEvent();
             ...
             if((id==MouseEvent.MOUSE_PRESSED)&&(buttonMask==MouseEvent.BUTTON1_MASK))
                      {
                       //disable the rotator when left mouse button is pressed so that the rotation
                       //  will stop
                      	rotator.setEnable(false);
                      	x_start=x;
                        y_start=y;
             ...
             if((id==MouseEvent.MOUSE_RELEASED)&&(buttonMask==MouseEvent.BUTTON1_MASK))
                      {
                      	...
                      	y_end=y;
             	        x_end=x;
             	         //calculate the time between the mouse pressed and the mouse released
