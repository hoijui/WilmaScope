...
public class DegreeLayout implements LayoutEngine {
  ...
  public void calculateLayout() {
    ...
    if (iterations == 500) {
      for (int i = 0; i != L.size(); ++i) {
        Node n = (Node) L.get(i);
        n.getLayout().setFixedPosition(true);
      }
    ...
    if (VariableForces.edgeForce) {
      ...
      for (Edge currentEdge : edges) {
        DegreeEdgeLayout edgeLayout = (DegreeEdgeLayout) currentEdge
            .getLayout();
    ...
    /*
     * // repelling edges version if (VariableForces.edgeForce) { // Compute
     * edge forces EdgeList edges = root.getInternalEdges(); float damping =
     * 2.0f; // damping >= 2 for (edges.resetIterator(); edges.hasNext();) {
     * Edge currentEdge = edges.nextEdge(); DegreeEdgeLayout edgeLayout =
     * (DegreeEdgeLayout)currentEdge.getLayout(); float resilience =
     * VariableForces.edgeResilience; float idealLength =
     * edgeLayout.idealLength;
     * 
     * Node start = currentEdge.getStart(); Node end = currentEdge.getEnd();
     * 
     * Vector3f repelForce = new Vector3f(); repelForce.sub(start.getPosition(),
     * end.getPosition()); float distance = repelForce.length(); if (distance !=
     * 0) { float repelFactor =
     * VariableForces.edgeResilience/(distance*distance);
     * 
  ...
  private void partition() {
    ...
    for (Node n : nodes) {
      n.getLayout().setFixedPosition(false);
