...
public class CopyCluster extends GraphModifier {
  ...
  public void modify(Cluster cluster) {
    ...
    c1.setLayoutEngine(ForceLayout.createDefaultClusterForceLayout(c1
        .getCluster()));
    try {
    ...
    if (levels) {
      root.getLayoutEngine().getProperties().setProperty("Levels",
          "" + (copyCount + 1));
      root.getLayoutEngine().resetProperties();
      // place contents of original cluster on level 0
      c1.getLayoutEngine().getProperties().setProperty("Levels",
          "" + (copyCount + 1));
      c1.getLayoutEngine().getProperties()
      .setProperty("LevelSeparation", "2");
    ...
    for (int i = 1; i <= copyCount; i++) {
      ...
      c2.setLayoutEngine(ForceLayout.createDefaultClusterForceLayout(c2
          .getCluster()));

      if (levels) {
        c2.getLayoutEngine().getProperties().setProperty("Levels",
            "" + (copyCount + 1));
        c2.getLayoutEngine().getProperties()
            .setProperty("LevelSeparation", "2");
