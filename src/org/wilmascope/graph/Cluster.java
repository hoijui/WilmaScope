...
/**
 * a Cluster is a set of Nodes ({@link Node}) and Edges ({@link Edge})
 * sharing the same ({@link LayoutEngine}). This class is the main interface
 * to the graph package for adding {@link GraphElement}s to a graph hierarchy.
 * 
 * @author Tim Dwyer
 * @version $Id$
 */
public class Cluster extends Node {
  ...
  /**
   * Add an internal edge to this cluster... If the edge is already a member of
   * this cluster it will be removed and then added again. Nothing like brute
   ...
   * References to the original graph elements are kept in userFacade. Does not
   * recurse down clusters.
   * 
   * @param original
   */
  ...
  private void calcRadius() {
    ...
    for (Node member : nodes) {
      tmpRadius = getPosition().distance(member.getPosition())
          + member.getView().getRadius();
   ...
   *          super.reposition(delta); if(expanded) { nodes.reposition(delta); } }
   */
  /**
  ...
  /**
   * On adding or removing a node from the cluster the mass of the cluster will
   * change, and a positive or negative contribution to the mass will have to be
   * added, not only to this cluster, but to parent clusters as well. The latter
   * is handled by {@link #setMass}
   */
  ...
  public void addBalancedEventListener(BalancedEventListener l) {
    WeakReference<BalancedEventListener> wr = new WeakReference<BalancedEventListener>(
        l);
    if (balancedEventListeners == null) { // lazy instantiation might save a bit
                                          // of memory
  ...
  /**
   * Finds an acyclic subset Ea of this cluster's edges E such that |Ea|>=|E|/2
   * using a simple greedy heuristic (Berger and Shor, 1990).
   *  
  ...
  /**
   * Uses a tricky heuristic (Eades et al. 1993) to find an acyclic subset (Ea) 
   * of this cluster's edges (E) such that:
   * |Ea|>=|E|/2+|V|/6
   *  
   * @return acyclic subset of edges in this cluster
   */
