...
/**
 * Generates a tree.
 * Proceeds by adding a random number of children to a random node of the
 * current tree. The probability of choosing a node is proportional to the
 * number of free spaces for its children. Generation can be limited by the
 * number of nodes, the maximum degree and optionally the tree height.
 * The root node can be forced to have a given number of children.
 *
...
System.out.println("Insert " + numChildren);
      ...
      /* Check if we've reached the end */
        if (nodes == numNodes || insertIndex == parentList.size() || parentList.get(insertIndex).node != parentTreeNode.node)
        {
