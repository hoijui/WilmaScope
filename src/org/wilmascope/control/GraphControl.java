...
public class GraphControl extends ObservableLayout {
	...
	public abstract class GraphElementFacade {
		...
		public void delete() {
			if (element instanceof org.wilmascope.graph.Edge) {
				...
				allEdges.removeAll(((org.wilmascope.graph.Node) element)
						.getEdges());
				allNodes.remove((org.wilmascope.graph.Node) element);
			}
			if (element instanceof org.wilmascope.graph.Cluster) {
				NodeList nodes = ((org.wilmascope.graph.Cluster) element)
						.getAllNodes();
	...
	public class Edge extends GraphElementFacade implements org.wilmascope.control.PickClient {
		...
		public void setProperty(String key, String value) {
			edge.getProperties().setProperty(key, value);
			edge.getLayout().resetProperties();
		}
	...
	public class Cluster extends Node implements org.wilmascope.control.PickClient {
		...
		public void setBalancedEventClient(BalancedEventClient client) {
			((ForceLayout) cluster.getLayoutEngine())
					.setBalancedEventClient(client);
		}
			...
			synchronized (gc) {
				try {
					...
					WilmaMain.showErrorDialog(
							"Couldn't get default Node View!", ex);
					return null;
				}
			}
		}
	...
	public GraphControl(int xsize, int ysize) {
		...
		try {
			...
			System.err.println("Couldn't load plugins because "
					+ ex1.getMessage());
		...
		try {
			...
			viewManager.getClusterViewRegistry().setDefaultView(
					"Spherical Cluster");
		} catch (ViewManager.UnknownViewTypeException e) {
			WilmaMain.showErrorDialog(
					"Unknown ViewType!  Fatal Error, stopping.", e);
			System.exit(1);
		}
