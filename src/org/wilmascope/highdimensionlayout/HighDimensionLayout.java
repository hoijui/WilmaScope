...
public class HighDimensionLayout implements LayoutEngine {
	...
	public void calculateLayout()
	{
		...
		for (int i = 0; i != m; ++i)
		{
		    ...
		    {
		    	Node bfsNode = nodes.nextNode();
				System.out.println(bfsNode.getPosition() + "  " + ((HDNodeLayout)bfsNode.getLayout()).graphTheoreticDist);
		    }*/
		...
		// eigenvectors are columns of matrix, sorted according to the corresponding eigenvalue in ascending order
		
		// get the first k eigenvectors (those that correspond to the largest eigenvalues)
	...
	// performs breadth first search starting from pivot node and stores graph-theoretic distance from pivot in each nodes HDNodeLayout
	...
	{	
	    ...
	    for (nodes.resetIterator(); nodes.hasNext();)
	    {
	      ((HDNodeLayout)(nodes.nextNode().getLayout())).setPos(); 
	    }
