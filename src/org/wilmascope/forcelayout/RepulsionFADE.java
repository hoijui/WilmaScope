  ...
  public void addNode(Node node, OctTreeCell cell) {
    ...
    if (cell.isFull()) {
      ...
      if (node.getPosition().equals(movingNode.getPosition())) {
        ...
        Point3f moveOff = new Point3f(rand.nextFloat() * (root.width / 2.0f)
            - (root.width / 4.0f), rand.nextFloat() * (root.width / 2.0f)
            - (root.width / 4.0f), rand.nextFloat() * (root.width / 2.0f)
            - (root.width / 4.0f));
    ...
    if (cell.isSplit()) {
      cell.centreOfMass.x = ((((float) cell.count) * cell.centreOfMass.x) + node
          .getPosition().x)
          / (((float) cell.count) + 1.0f);
      cell.centreOfMass.y = ((((float) cell.count) * cell.centreOfMass.y) + node
          .getPosition().y)
          / (((float) cell.count) + 1.0f);
      cell.centreOfMass.z = ((((float) cell.count) * cell.centreOfMass.z) + node
          .getPosition().z)
          / (((float) cell.count) + 1.0f);
    ...
    if (distance != 0) {
      ...
      if (M <= accuracyParameter || cell.isLeaf()) {
        float repelFactor = ((float) cell.count)
            * strengthConstant / (distance * distance);
