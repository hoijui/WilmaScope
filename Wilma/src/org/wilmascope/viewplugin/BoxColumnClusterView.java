...
public class BoxColumnClusterView extends ColumnClusterView {
  public void draw() {
    try {
      ...
      // gap is simply to make our cluster slightly smaller than the nodes, 
      // so that the nodes are drawn in front of the cluster and we don't get wierd affects due to
  ...
  public void setLabel(String[] labels) {
    ...
    for (int i = 0; i < labels.length; i++) {
      Text2D text =
        new Text2D(labels[i], labelColour, "dummy", 60, java.awt.Font.PLAIN);
      makePickable(text);
