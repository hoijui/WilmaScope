...
public class TubeNodeView extends NodeView implements SizeAdjustableNodeView {
  ...
  public void draw() {
    double height = 0.5;
    if (getNode().getLayout() instanceof NodeColumnLayout) {
    ...
    setResizeTranslateTransform(new Vector3d(1, 1, height), new Vector3f(
        getNode().getPosition()));
  }
  ...
  protected void init() {
    ...
    for (int i = 0; i < topPoints.length; i++) {
      scaledTopPoints[i] = new Point3f(topPoints[i].x * radius, topPoints[i].y
          * radius, topPoints[i].z);
    }

    scaledBottomPoints = new Point3f[bottomPoints.length];
    for (int i = 0; i < bottomPoints.length; i++) {
      scaledBottomPoints[i] = new Point3f(bottomPoints[i].x * radius,
          bottomPoints[i].y * radius, bottomPoints[i].z);
    }
    for (int i = 0; i < tubePoints.length; i++) {
      if (i % 2 == 0) {
        taperedTubePoints[i] = new Point3f(tubePoints[i].x * radius,
            tubePoints[i].y * radius, tubePoints[i].z);
      } else {
        taperedTubePoints[i] = new Point3f(tubePoints[i].x * radius,
            tubePoints[i].y * radius, tubePoints[i].z);
      }
    ...
    normalGenerator.generateNormals(gi);
    topGeometryArray = new TriangleFanArray(scaledTopPoints.length,
        GeometryArray.COORDINATES | GeometryArray.BY_REFERENCE
            | GeometryArray.NORMALS, topStripCounts);
    ...
    normalGenerator.generateNormals(gi);
    bottomGeometryArray = new TriangleFanArray(scaledBottomPoints.length,
        GeometryArray.COORDINATES | GeometryArray.BY_REFERENCE
            | GeometryArray.NORMALS, bottomStripCounts);
