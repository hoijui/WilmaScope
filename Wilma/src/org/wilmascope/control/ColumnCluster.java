...
public class ColumnCluster {
	public ColumnCluster(
		...
		float initValue,
		float bottomRadius,
		int initLevel, String nodeViewType) {
		this.root = root;
		lastValue = this.initValue = initValue;
		try {
			ViewManager.getInstance().getClusterViewRegistry().setDefaultView(
				"Column Cluster");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	class ColTubeLayout implements Layout {
      ...
      ((TubeView) n.getView()).setEndRadii(lastTopRadius, radius);
			return n;
		}
		public int getNextLevel() {
			return (
				(org.wilmascope.columnlayout.ColumnLayout) column
					.getCluster()
					.getLayoutEngine())
				.getStrataCount();
		}
		...
		public GraphControl.NodeFacade addNode(float radius) {
			...
			if (lastTopNode != null) {
				...
				GraphControl.EdgeFacade e =
					column.addEdge(lastTopNode, n, "Tapered Edge");
