...
public class SplineTubeEdgeView extends EdgeView implements View2D,org.wilmascope.dotlayout.Spline {
  //
  ...
  public void draw2D(Renderer2D r, Graphics2D g) {
    ...
    } else g.setStroke(new BasicStroke(thickness));
  ...
  private void arrowPosition(EdgeClient.ArrowPosition arrowPos, Point3f start, Point3f end) {
      ...
      xScale * (((float)(arrowPos.position.x - bounds.x))/(float)bounds.width - 0.5f),
      yScale * (((float)(arrowPos.position.y - bounds.y))/(float)bounds.width - 0.5f),
      zLevel);
    Point2D.Float p;
