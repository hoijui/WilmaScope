...
/**
 * Grid provides a matrix of node arrays, where each node array provides a
 * list of nodes located in a particular region of space (a cell).
 * Thus the nodes in the neighbourhood of a particular node can be easily found
 * as the contents of the arrays in the neighbouring cells.
 * Originally there was a 2D implementation of the following interface as
 * well as the 3D.  Now it's redundant but I'll leave it here in case
 * someone feels the need to reimplement a super efficient 2D version again.
 */
interface Grid {
  /**
  ...
  /**
   * if the node positions needed to be scaled to fit the grid
   * then the scale factor used can be
...
class Grid3D implements Grid{
   ...
   * cells of dimensions R.  Nodes are then assigned to their neighbourhood cell.
   * If the graph takes up more space than the dimensions specified by the argument
   * then the node positions are scaled to fit.  In this case the k value
   * (natural spring length used for the attractive force)
   * should also be scaled... hence the scale can be obtained by the getScale
   * method.
   */
  Grid3D(Dimension dim, MultiScaleNodeLayout nodes[], float R) {
    ...
    maxCell.set((int)Math.ceil((float)graphDim.x / R),
                (int)Math.ceil((float)graphDim.y / R),
    ...
    for (int i = 0; i < nodes.length ; i++) {
      ...
      v.cell.set(
          (int)((v.position.x - min.x)/R),
          (int)((v.position.y - min.y)/R),
  ...
  public void applyForceToNeighbourhood(Force f, MultiScaleNodeLayout v) {
    for (int i = v.cell.x - 1; i <= v.cell.x + 1; i++) {
      if(i < 0 || i >= maxCell.x) continue;
      for (int j = v.cell.y - 1; j <= v.cell.y + 1; j++) {
        if(j < 0 || j >= maxCell.y) continue;
        for (int k = v.cell.z - 1; k <= v.cell.z + 1; k++) {
          if(k < 0 || k >= maxCell.z) continue;
...
public class MultiScaleGraph {
  ...
  /**
   * Computes a simple matching of the graph.
   * nodes are visited at random and paired with their smallest mass neighbour
   ...
   * @param x separation between nodes
   * @param mass weight or mass of node
   */
  ...
  float repulsiveForce_xsquared(float xsquared, float mass) {
    return (xsquared<=Rsquared) ?
        repulsiveForceConstant * mass * ksquared / xsquared : 0;
  }
  ...
  float fl_squared(float xsquared, float d, float mass) {
    return springForceConstant * (xsquared - ksquared) / (k * d * d)
        + repulsiveForce_xsquared(xsquared,mass);
  }
  ...
  boolean layout(Dimension dim) {
    ...
    for (int i = 0; i < nodes.length ; i++) {
      ...
      /* replacing the above line with the following would apply global force
       * to all nodes instead of just the neighbourhood... real slow
    ...
    } for (int i = 0; i < nodes.length ; i++) { v = nodes[i];
