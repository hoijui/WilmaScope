 ...
 * the neighbourhood of a particular node can be easily found as the contents
 * of the arrays in the neighbouring cells. Originally there was a 2D
...
class Grid3D implements Grid {
	 ...
	 * the argument then the node positions are scaled to fit. In this case the k
	 * value (natural spring length used for the attractive force) should also be
	 * scaled... hence the scale can be obtained by the getScale method.
	 */
	Grid3D(Dimension dim, MultiScaleNodeLayout nodes[], float R) {
		...
		maxCell.set((int) Math.ceil((float) graphDim.x / R), (int) Math
				.ceil((float) graphDim.y / R), (int) Math.ceil((float) graphDim.z / R));
		array = new Object[maxCell.x + 1][maxCell.y + 1][maxCell.z + 1];
		// centre all nodes around the origin
		Vector3f shift = new Vector3f((-max.x - min.x) / 2, (-max.y - min.y) / 2,
				(-max.z - min.z) / 2);
		min.add(shift);
		max.add(shift);
		for (int i = 0; i < nodes.length; i++) {
			...
			v.cell.set((int) ((v.position.x - min.x) / R),
...
public class MultiScaleGraph {
	...
	float ksquared;
	int level;
	MultiScaleGraph(MultiScaleNodeLayout[] nodes, MultiScaleEdgeLayout[] edges,
			int level) {
		this.nodes = nodes;
		this.edges = edges;
		this.level = level;
	...
	private MultiScaleNodeLayout luckyDip(ArrayList list) {
		return (MultiScaleNodeLayout) list
				.get((int) ((float) Math.random() * (float) list.size()));
	}
	...
	MultiScaleGraph coursenedGraph() {
		...
		for (int i = 0; i < edges.length; i++) {
			...
			if (e.u.parent != e.v.parent) {
				MultiScaleEdgeLayout f = new MultiScaleEdgeLayout(e.u.parent,
						e.v.parent);
		...
		MultiScaleNodeLayout[] courseNodeArray = new MultiScaleNodeLayout[courseNodes
				.size()];
		MultiScaleEdgeLayout[] courseEdgeArray = new MultiScaleEdgeLayout[courseEdges
				.size()];
	...
	float repulsiveForce_xsquared(float xsquared, float mass) {
		return (xsquared <= Rsquared) ? repulsiveForceConstant * mass * ksquared
				/ xsquared : 0;
	}
	/**
	...
	void randomLayout(Dimension dim) {
		for (int i = 0; i < nodes.length; i++) {
			nodes[i].position.set(dim.width * (float) Math.random(), dim.height
					* (float) Math.random(), 0);
		}
	}
	float randomLength() {
	...
	boolean layout(Dimension dim) {
		...
		for (int i = 0; i < nodes.length; i++) {
			 ...
			 * to all nodes instead of just the neighbourhood... real slow for (int j = 0;
			 * j < nodes.length; j++) { u = nodes[j]; fglobal.apply(u,v); }
			 */
